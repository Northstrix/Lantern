...
const int MENU_ITEMS_PER_PAGE = 6;
const int MENU_ITEM_SPACING = 20;
const int MENU_TEXT_SIZE = 2;
String menu_items[] = {
...
int menu_shift_down = 12; // Pixels to shift menu items down
...
byte get_nunchuk_input() {
  ...
  while (!success) {
    ...
    if (currentTime - new_time_millis >= 1000) {
      ...
      if (colorSwap) {
        display_centered_highlighted_hebrew_text("Nunchuk", 166, DEFAULT_TEXT_COLOR, DEFAULT_RED_COLOR);
        display_centered_highlighted_hebrew_text("Disconnected", 186, DEFAULT_TEXT_COLOR, DEFAULT_RED_COLOR);
      } else {
        display_centered_highlighted_hebrew_text("Nunchuk", 166, DEFAULT_RED_COLOR, DEFAULT_TEXT_COLOR);
        display_centered_highlighted_hebrew_text("Disconnected", 186, DEFAULT_RED_COLOR, DEFAULT_TEXT_COLOR);
      }
    }
  ...
  if (nunchukWasDisconnected == true) {
    display_centered_highlighted_hebrew_text("Nunchuk", 166, COMMON_FILL_COLOR, COMMON_FILL_COLOR);
    display_centered_highlighted_hebrew_text("Disconnected", 186, COMMON_FILL_COLOR, COMMON_FILL_COLOR);
...
void display_centered_highlighted_hebrew_text(String text, int h, uint16_t hghl_color, uint16_t text_color) {
...
void setup() {
  ...
  tft.setTextSize(2);
  display_centered_highlighted_hebrew_text("Addressable", 10, DEFAULT_HIGHLIGHT_COLOR, DEFAULT_TEXT_COLOR);
  display_centered_highlighted_hebrew_text("RGB LED Strip", 30, DEFAULT_HIGHLIGHT_COLOR, DEFAULT_TEXT_COLOR);
  display_centered_highlighted_hebrew_text("Controller", 50, DEFAULT_HIGHLIGHT_COLOR, DEFAULT_TEXT_COLOR);
  Serial.begin(115200);
  draw_soil();
  tft.setTextSize(1);
  display_centered_highlighted_hebrew_text("github.com/Northstrix/Lantern", 230, DEFAULT_HIGHLIGHT_COLOR, DEFAULT_TEXT_COLOR);
  tft.setTextSize(2);
  ...
  while (!wii_nunchuk.connect()) {
    colorSwap = !colorSwap;
    if (colorSwap) {
      display_centered_highlighted_hebrew_text("Connect Nunchuk", 180, DEFAULT_TEXT_COLOR, DEFAULT_RED_COLOR);
    } else {
      display_centered_highlighted_hebrew_text("Connect Nunchuk", 180, DEFAULT_RED_COLOR, DEFAULT_TEXT_COLOR);
    }
    delay(1000);
  }
  ...
  while (break_start_lp == false) {
    ...
    if (currentTime - new_time_millis >= 1000) {
      ...
      if (colorSwap) {
        display_centered_highlighted_hebrew_text("Press Any Key", 140, DEFAULT_HIGHLIGHT_COLOR, DEFAULT_TEXT_COLOR);
      } else {
        display_centered_highlighted_hebrew_text("Press Any Key", 140, DEFAULT_TEXT_COLOR, DEFAULT_HIGHLIGHT_COLOR);
      }
    }
...
void loop() {
  ...
  if (input_data != 0) {
    ...
    if (menu_pos == 33) {
      tft.setTextSize(4);
      display_centered_highlighted_hebrew_text(String(brightness), 110, DEFAULT_HIGHLIGHT_COLOR, DEFAULT_TEXT_COLOR);

      // Handle brightness adjustment
      if (input_data == 130) { // Right
        // Erase old value
        display_centered_highlighted_hebrew_text(String(brightness), 110, COMMON_FILL_COLOR, COMMON_FILL_COLOR);
